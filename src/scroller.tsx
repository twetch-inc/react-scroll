...
export class InfiniteScroller extends React.Component<Props, State> {
	...
	public componentDidMount() {
		...
		const guesstimatedItemCountPerPage = Math.ceil(
			this.divDom.clientHeight / this.props.itemAverageHeight
		);
		this.projector = new Projector(
			this.props.containerHeight,
			guesstimatedItemCountPerPage,
			5,
			this.props.items,
			this.props.itemAverageHeight,
			this.props.cache
		);
	...
	public render() {
		...
		return (
			<div
				...
				ref={div => (this.divDom = div!)}
				style={style}
				onScroll={this.onScroll}
			>
				<div
					ref={div => (this.upperDom = div!)}
					style={{ height: this.state.upperPlaceholderHeight }}
				/>
				...
				<div
					ref={div => (this.underDom = div!)}
					style={{ height: this.state.underPlaceholderHeight }}
				/>
			</div>
		);
	}
			...
			if (startIndex > 0) {
				if (finalHeight < 0) {
					if (isIos) {
						...
						this.isAdjusting = true;
						this.setState(
							{ upperPlaceholderHeight: this.state.upperPlaceholderHeight + estimateHeight },
							() => {
